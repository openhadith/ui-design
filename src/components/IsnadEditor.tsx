        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map((l, i) => {
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Text style={{ width: 20, textAlign: 'center', fontSize: 11, color: c.inkGhost }}>
                    {toAr(i + 1)}
                  </Text>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: dot, flex: 'none' }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <Text strong ellipsis style={{ ...AMIRI, fontSize: 15, display: 'block' }}>
        ...
        <Space.Compact style={{ width: '100%' }}>
          ...
          <Select
            ...
            options={hits.map((h) => ({
              ...
              label: (
                <span>
                  <span style={{ ...AMIRI, fontSize: 14 }}>{h.shohra || h.name}</span>
                  {h.deathdate && <Text type="secondary" style={{ fontSize: 10.5 }}> · ت {toAr(h.deathdate)}</Text>}
                </span>
              ),
            }))}
          />
