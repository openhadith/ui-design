      ...
      <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', padding: '16px 20px 32px' }}>
        <Row gutter={[14, 14]}>
          ...
          <Col xs={24} xl={17}>
            ...
            >
              <Text style={{ color: 'rgba(255,255,255,.72)', fontSize: 11.5 }}>
                {todayLabel()}
              </Text>
              <Title level={3} style={{ color: '#fff', margin: '4px 0 6px' }}>
                بەخێربێیتەوە، {user.name}
              </Title>
              <Text style={{ color: 'rgba(255,255,255,.85)', fontSize: 12.5 }}>
                {today.done > 0
                ...
                <Link href={`/hadith/${resume[0].entity_id}`}>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <Text style={{ color: 'rgba(255,255,255,.75)', fontSize: 10.5, display: 'block' }}>
                        بگەڕەوە بۆ کارە ناتەواوەکەت
                      </Text>
                      <span
                        style={{
                          ...AMIRI, fontSize: 14, color: '#fff',
        ...
        <Row gutter={[14, 14]} style={{ marginTop: 14 }}>
          <Col xs={24} lg={15}>
            <Card
              ...
              extra={<Text type="secondary" style={{ fontSize: 11 }}>کۆ: {toAr(week.reduce((s, w) => s + w.count, 0))} بڕیار</Text>}
            >
                ...
                {week.map((w) => {
                  ...
                  return (
                    <div key={w.day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <Text style={{ fontSize: 10, color: isPeak ? c.emerald : c.inkGhost, fontWeight: isPeak ? 700 : 400 }}>
                        {toAr(w.count)}
                      </Text>
                ...
                <Progress
                  ...
                  format={(p) => (
                    <div style={{ lineHeight: 1.2 }}>
                      <div style={{ fontSize: 19, fontWeight: 700, color: c.emerald }}>{toAr(p ?? 0)}٪</div>
                      <div style={{ fontSize: 9.5, color: c.inkGhost }}>بێ کێشە</div>
                    </div>
                  )}
                />
                <div style={{ flex: 1, fontSize: 11.5, lineHeight: 2.1, color: c.inkMuted }}>
                  ...
                  <Link href="/admin/quality" style={{ fontSize: 11 }}>بینینی هەموو کێشەکان →</Link>
                </div>
              </div>
            </Card>
          </Col>
        ...
        <Row gutter={[14, 14]} style={{ marginTop: 14 }}>
          <Col xs={24} lg={12}>
            <Card
              ...
              extra={<Link href="/admin/audit" style={{ fontSize: 11 }}>هەموو</Link>}
              styles={{ body: { padding: 0 } }}
            >
              {recentEdits.length === 0 ? (
                <div style={{ padding: 18, textAlign: 'center' }}>
                  <Text type="secondary" style={{ fontSize: 11.5 }}>هێشتا دەستکارییەک نەکراوە</Text>
                </div>
              ) : (
                recentEdits.map((e) => (
                  <div key={e.id} style={ROW}>
                    ...
                    {e.entity_id && (
                      <Text style={{ fontSize: 10.5, color: c.inkGhost, flex: 'none' }}>
                        <RecordId id={e.entity_id} />
                      </Text>
                    )}
                    <Text style={{ fontSize: 10.5, color: c.inkGhost, flex: 'none' }}>{agoKu(e.created_at)}</Text>
                  </div>
                ))
              )}
            </Card>
          ...
          <Col xs={24} lg={12}>
            <Card size="small" title="پینکراوەکان" styles={{ body: { padding: 0 } }}>
              {pinned.length === 0 ? (
                <div style={{ padding: 18, textAlign: 'center' }}>
                  <Text type="secondary" style={{ fontSize: 11.5 }}>هیچ ڕەکۆردێکی پینکراو نییە</Text>
                </div>
              ) : (
                pinned.map((p) => (
                  <Link key={p.entity_id} href={`/hadith/${p.entity_id}`} style={{ color: 'inherit' }}>
                    <div style={ROW}>
                      <StarFilled style={{ color: c.gold, fontSize: 12 }} />
                      <span
                        style={{
                          flex: 1, minWidth: 0, ...AMIRI, fontSize: 14,
                          display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical',
                          overflow: 'hidden',
                        }}
                      >
                      ...
                      <Text style={{ fontSize: 10.5, color: c.inkGhost, flex: 'none' }}>{p.detail}</Text>
                    </div>
                  </Link>
                ))
              )}
        ...
        <Card size="small" title="چالاکی تیم">
          <Timeline
            ...
            items={activity.map((a) => {
              ...
              return {
                icon: (
                  <Avatar size={22} style={{ background: tone.bg, color: tone.fg, fontSize: 9, fontWeight: 600 }}>
          ...
          {leaders.map((l, i) => {
            ...
            return (
              <div key={l.name} style={ROW}>
                ...
                <Avatar size={24} style={{ background: tone.bg, color: tone.fg, fontSize: 10, fontWeight: 600 }}>
                  {initials(l.name)}
                </Avatar>
                <Text style={{ flex: 1, fontSize: 12 }}>{l.name}</Text>
  ...
  return (
    <Link href={href} style={{ flex: 1 }}>
      <Card size="small" hoverable style={{ height: '100%' }}
        ...
        <span style={{ color: c.emerald, fontSize: 15 }}>{icon}</span>
      ...
      <Link href={href}>
        <Card size="small" hoverable>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 26, fontWeight: 700, color: c.inkStrong, lineHeight: 1 }}>
              {toAr(value)}
            </span>
            {delta && (
              <Tooltip title={note}>
                <span style={{ fontSize: 11, fontWeight: 600, color: delta.good ? c.emerald : c.rust }}>
