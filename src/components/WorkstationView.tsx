    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        >
          ...
          {hadith?.book?.title && <Text type="secondary" style={{ fontSize: 11.5 }}>· {hadith.book.title}</Text>}
          {hadith?.hadithid && <Text type="secondary" style={{ fontSize: 11.5 }}>ژمارە {toAr(hadith.hadithid)}</Text>}
          <Button
            size="small"
            type="link"
            icon={<ExportOutlined />}
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '16px 20px 40px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          >
            <Input.TextArea
              ...
              style={{ ...AMIRI, fontSize: 17, lineHeight: 2, padding: '12px 16px' }}
            />
          </Card>
          ...
          <Card
            ...
            extra={
              <Space size={8}>
                ...
                <Text type="secondary" style={{ fontSize: 11 }}>{toAr(chain.length)} ڕاوی</Text>
                <Button
                  size="small"
                  icon={<EditOutlined />}
                  disabled={!can('edit')}
                ...
                <Link href={`/sanad?id=${id}`} style={{ fontSize: 11.5 }}>پشکنینی سەنەد</Link>
              </Space>
            }
          >
            {chain.length === 0 ? (
                      ...
                      <Link href={`/narrator/${link.rawyId}`}>
                        <Card size="small" hoverable style={{
                          ...
                          <Space size={6}>
                            ...
                            <Text strong ellipsis style={{ ...AMIRI, fontSize: 14, maxWidth: 170 }}>
                              {link.rawy?.Shohra || link.rawy?.Name}
                            </Text>
                          </Space>
                          <div>
                            <Text type="secondary" ellipsis style={{ fontSize: 10, maxWidth: 190 }}>
              ...
              {hadith!.assessments!.map((a) => (
                <div key={a.index} style={{ padding: '9px 14px', borderBottom: `1px solid ${c.lineSoft}` }}>
                  <Space size={8} style={{ marginBottom: 4 }}>
                    <Text strong style={{ fontSize: 11 }}>إسناد {toAr(a.index)}</Text>
                    {a.grades.map((g) => (
                      <TokenTag key={g} token={GRADE[gradeKeyOf(g)] ?? GRADE.unknown} title={hukmLabel(g) ?? undefined} />
                    ))}
                    <Text type="secondary" style={{ fontSize: 10.5 }}>{toAr(a.branches.length)} لق</Text>
                  </Space>
                  {a.sharh && <div style={{ ...AMIRI, fontSize: 14, lineHeight: 1.9 }}>{a.sharh}</div>}
                </div>
              ))}
            </Card>
          )}
          ...
          {hadith?.full_hadith && (
            <Card size="small" title="دەقی تەواو وەک تۆمارکراوە">
              <div dir="rtl" style={{ ...AMIRI, fontSize: 15, lineHeight: 2 }}>{hadith.full_hadith}</div>
            </Card>
          )}
        </div>
        ...
        >
          ...
          <Space style={{ marginInlineStart: 'auto' }}>
            {nextId && <Text type="secondary" style={{ fontSize: 10.5 }}>⌘↵ پاشەکەوت و دواتر</Text>}
            <Button
              type="primary"
              icon={<SaveOutlined />}
      ...
      >
              ...
              { key: 's', label: 'دۆخ', children: <TokenTag token={status} /> },
              {
                ...
                children: review?.assignee_name ? (
                  <Space size={6}>
                    ...
                    <Text style={{ fontSize: 11.5 }}>{review.assignee_name}</Text>
                  </Space>
        ...
        {hadith?.hukmText && (
          <Card size="small" title="حوکمی تۆمارکراو">
            <div style={{ ...AMIRI, fontSize: 14, lineHeight: 1.9 }}>{hadith.hukmText}</div>
          </Card>
        )}
          ...
          {revisions.length === 0 ? (
            ...
            <Timeline
              items={revisions.map((rv) => ({
                content: (
                  <div>
                    <Text strong style={{ fontSize: 11.5 }}>{rv.author_name ?? 'سیستەم'}</Text>{' '}
                    <Text type="secondary" style={{ fontSize: 10.5 }}>{agoKu(rv.created_at)}</Text>
                    {rv.note && <div><Text type="secondary" style={{ fontSize: 10.5 }}>{rv.note}</Text></div>}
                  </div>
                ),
              }))}
            />
